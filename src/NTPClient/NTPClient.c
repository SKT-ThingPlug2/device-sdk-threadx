...
int NTPSetTimer(void)
{
    ...
    {
        g_sf_comms0.p_api->write(g_sf_comms0.p_ctrl, "NTP Socket Create error\r\n", sizeof("NTP Socket Create error\r\n"), TX_NO_WAIT);
        return -1;
    }
    g_sf_comms0.p_api->write(g_sf_comms0.p_ctrl, "NTP Socket Create ok\r\n", sizeof("NTP Socket Create ok\r\n"), TX_NO_WAIT);
    ...
    if ( result != NX_SUCCESS )
    {
        g_sf_comms0.p_api->write(g_sf_comms0.p_ctrl, "NTP Time Request error\r\n", sizeof("NTP Time Request error\r\n"), TX_NO_WAIT);
	goto socket_error;
    }
    result = nx_packet_release(packet_ptr);
    g_sf_comms0.p_api->write(g_sf_comms0.p_ctrl, "NTP Time Request ok\r\n", sizeof("NTP Time Request ok\r\n"), TX_NO_WAIT);

    ULONG len = sizeof(NTPPacket);
    ULONG copied_idx = 0;
    Timer t;
    countdown_ms(&t , 2000);
    ...
    if( len != 0 ) {
        g_sf_comms0.p_api->write(g_sf_comms0.p_ctrl, "NTP Time Receive error\r\n", sizeof("NTP Time Receive error\r\n"), TX_NO_WAIT);
	goto socket_error;
    }
    g_sf_comms0.p_api->write(g_sf_comms0.p_ctrl, "NTP Time Receive ok\r\n", sizeof("NTP Time Receive ok\r\n"), TX_NO_WAIT);
